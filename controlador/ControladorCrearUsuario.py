...
class ControladorCrearUsuario(qtw.QMainWindow):
    ...
    def __init__(self, mode):
        super().__init__()
        self.vista = VistaCrearUsuario(mode)
    ...
    def create_user(self):
        ...
        if valid_data:
            ...
            # Show error message
            self.vista.message.setText('Datos incorrectos o incompletos.\nRevisar que las contraseñas coincidan.\n\
                Llenar los valores con *.')
            self.vista.message.setStyleSheet('color: red')
    ...
    def validate_inputs(self):
        ...
        if self.vista.inputs['Tipo de usuario*'].isChecked() and (self.letters_numbers_special.match(self.vista.inputs['Nombre de usuario*'].text()) is None or\
        ...
        if self.numbers.match(self.vista.inputs['Teléfono*'].text()) is None or len(self.vista.inputs['Teléfono*'].text()) !=10:
            return False

        if self.letters_spaces.match(self.vista.inputs['Nombre(s)*'].text()) is None or self.letters_spaces.match(self.vista.inputs['Apellido paterno*'].text()) is None or\
            self.letters_spaces.match(self.vista.inputs['Apellido materno'].text()) is None or self.letters_numbers_spaces.match(self.vista.inputs['Dirección'].text()) is None or\
    ...
    def save_user(self):
        try:
            if self.vista.inputs['Tipo de usuario*'].isChecked():
                user = ModeloUsuario(self.vista.inputs['Nombre de usuario*'].text(), self.vista.inputs['Contraseña*'].text())
                terapeuta_data = ModeloTerapeuta(self.)
            else:
                user = ModeloUsuario(self.vista.inputs['Nombre de usuario*'].text(), self.vista.inputs['Contraseña*'].text(), 0)

            self.Consulta.guardarUsuario(user)
