...
class ControladorCrearUsuario:
    ...
    def create_user(self):
        ...
        if valid_data:
            ...
            resultado = self.Consulta.consultarUsarioPorNombre(
                self.vista.inputs["Nombre de usuario*"].text()
            )
            ...
            # Show error message
            self.vista.message.setText(
                "Llenar los valores con *. Revisar que las contraseñas coincidan.\n\
    ...
    def validate_inputs(self):
        for values in self.vista.inputs.values():
            if (
                values == self.vista.inputs["Apellido materno"]
                or values == self.vista.inputs["Dirección"]
                or values == self.vista.inputs["Localidad"]
                or values == self.vista.inputs["Tipo de usuario*"]
                or values == self.vista.inputs["Género*"]
                or values == self.vista.inputs["Fecha de nacimiento*"]
            ):
                continue
            else:
                if (
                    values != self.vista.inputs["Nombre de usuario*"]
                    and values != self.vista.inputs["Contraseña*"]
        ...
        if self.vista.inputs["Tipo de usuario*"].isChecked() and (
            self.letters_numbers_special.match(
                self.vista.inputs["Nombre de usuario*"].text()
            )
            is None
            or self.letters_numbers_special.match(
                self.vista.inputs["Contraseña*"].text()
            )
            is None
            or self.letters_numbers_special.match(
                self.vista.inputs["Confirmar contraseña*"].text()
            )
            is None
            or len(self.vista.inputs["Contraseña*"].text()) < 6
        ):
            return False
        ...
        if (
            self.vista.inputs["Contraseña*"].text()
            != self.vista.inputs["Confirmar contraseña*"].text()
            and self.vista.inputs["Tipo de usuario*"].isChecked()
        ):
            return False

        if (
            self.numbers.match(self.vista.inputs["Teléfono*"].text()) is None
            or len(self.vista.inputs["Teléfono*"].text()) != 10
        ):
            return False

        if (
            self.letters_spaces.match(self.vista.inputs["Nombre(s)*"].text()) is None
            or self.letters_spaces.match(self.vista.inputs["Apellido paterno*"].text())
            is None
            or self.letters_spaces.match(self.vista.inputs["Apellido materno"].text())
            is None
            or self.letters_numbers_spaces.match(self.vista.inputs["Dirección"].text())
            is None
            or self.letters_numbers_spaces.match(self.vista.inputs["Localidad"].text())
            is None
        ):
            return False
    ...
    def load_user(self):
        try:
            ...
            self.vista.inputs["Fecha de nacimiento*"].setDate(
                datetime.datetime.strptime(user[0][5], "%d/%m/%Y").date()
            )
