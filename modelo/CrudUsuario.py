...
class CrudUsuario(Conexion):
    ...
    def consultarUsuario(self, usuario: ModeloUsuario):
        try:
            self.cursor.execute(
                "SELECT * FROM usuarios WHERE nombreUsuario = '{}' and password ='{}'".format(
            ...
            if len(resultado) > 0 and resultado[0][2] == "1":
                self.cursor.execute(
                    "SELECT idTerapeuta FROM terapeuta WHERE usuarios_nombreUsuario = '{}'".format(
                        resultado[0][0]
                    )
                )
    ...
    def consultarUsarioPorNombre(self, nombreUsuario):
        try:
            self.cursor.execute(
                "SELECT * FROM usuarios WHERE nombreUsuario = '{}'".format(
                    nombreUsuario
                )
            )
            resultado = self.cursor.fetchall()
    ...
    def guardarUsuario(
        self,
        data: [ModeloTerapeuta, ModeloPatient],
        type: int,
        idTerapist=None,
        ...
        if type == -1:
            ...
            self.cursor.execute(
                "INSERT INTO usuarios Values('{}', '{}', {})".format(
                    user.nombreUsuario, user.password, user.tipo
                )
            )
            self.cursor.execute(
            ...
            if type == 0:
                ...
                self.cursor.execute(
                    "INSERT INTO {} (nombre, ape_paterno, ape_materno, genero, fecha_nacimiento, localidad, calle,\
                    numero_contacto, Tutor_idTutor) Values('{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}')".format(
                        table,
                        data.nombre,
                        data.ape_paterno,
                        data.ape_materno,
                ...
                self.cursor.execute(
                    ...
                    numero_contacto='{}' WHERE idPaciente = {}".format(
                        table,
                        data.nombre,
                        data.ape_paterno,
                        data.ape_materno,
    ...
    def consultarPacientesAsociados(self, id: int):
        try:
            self.cursor.execute(
                "SELECT idPaciente FROM paciente WHERE Tutor_idTutor = '{}'".format(id)
            )
            id_pacientes = self.cursor.fetchall()
            self.lista_pacientes = ()
            for idOnly in id_pacientes:
                self.cursor.execute(
                    "SELECT * FROM paciente WHERE idPaciente = '{}'".format(idOnly[0])
                )
    ...
    def eliminarPacientes(self, id: int):
        try:
            self.cursor.execute(
                "DELETE FROM terapeuta_has_paciente WHERE paciente_idPaciente = '{}'".format(
                    id
                )
            )
            self.cursor.execute(
                "DELETE FROM sesionterapeutica WHERE Paciente_idPaciente = '{}'".format(
                    id
                )
            )
            self.cursor.execute(
                "DELETE FROM paciente WHERE idPaciente = '{}'".format(id)
            )
            self.db.commit()
        except Exception as err:
    ...
    def consultarPacientePorId(self, id: int):
        try:
            self.cursor.execute(
                "SELECT * FROM paciente WHERE idPaciente = '{}'".format(id)
            )
    ...
    def consultarTerapeutaPorId(self, id):
        try:
            self.cursor.execute(
                "SELECT * FROM terapeuta WHERE idTerapeuta = '{}'".format(id)
            )
            return self.cursor.fetchall()
        except Exception as err:
    ...
    def guardarEjercicio(self, idTerapeuta, idPaciente, valores, tiempo, descripcion):
        try:
            self.cursor.execute(
                "INSERT INTO sesionterapeutica (Terapeuta_idTrapeuta, Paciente_idPaciente, funcionCognitiva,\
                                tiempo, comentarios) VALUES ('{}', '{}', '{}', '{}', '{}')".format(
                    idTerapeuta, idPaciente, valores, tiempo, descripcion
                )
            )
            self.db.commit()
        except Exception as err:
